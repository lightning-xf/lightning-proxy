...
class _NodeEditDialogState extends State<NodeEditDialog> {
  ...
  void initState() {
    ...
    _concurrencyController = TextEditingController(text: node?.muxConcurrency?.toString() ?? '8');

    _nameController = TextEditingController(text: node?.name ?? '');
    _addressController = TextEditingController(text: node?.address ?? '');
    _portController = TextEditingController(text: node?.port?.toString() ?? '443');
    ...
    _serviceNameController = TextEditingController(text: node?.serviceName ?? '');
    _flowController = TextEditingController(text: node?.flow ?? '');
    _methodController = TextEditingController(text: _ssMethod);
    _encryptionController = TextEditingController(text: _protocol == 'vmess' ? _vmessSecurity : _vlessEncryption);
    _wgSecretKeyController = TextEditingController(text: node?.wgSecretKey ?? '');
    _wgPeerPublicKeyController = TextEditingController(text: node?.wgPeerPublicKey ?? '');
    _wgPreSharedKeyController = TextEditingController(text: node?.wgPreSharedKey ?? '');
    _wgLocalAddressController = TextEditingController(text: node?.wgLocalAddress?.join(',') ?? '10.0.0.2/32');
    _wgMtuController = TextEditingController(text: node?.wgMtu?.toString() ?? '1420');
    _wgKeepAliveController = TextEditingController(text: node?.wgKeepAlive?.toString() ?? '20');
  ...
  Widget build(BuildContext context) {
    ...
    return BackdropFilter(
      ...
      child: AlertDialog(
        backgroundColor: isDark ? Colors.black.withValues(alpha: 0.8) : Colors.white.withValues(alpha: 0.85),
            ...
            child: Column(
              ...
              children: [
                ...
                _buildDropdown<String>(
                  label: '协议',
                  value: _protocol,
                  items: ['vmess', 'vless', 'trojan', 'shadowsocks', 'socks', 'http', 'hysteria2', 'tuic', 'wireguard', 'dokodemo-door'],
                ...
                _buildTextField(controller: _nameController, label: '名称', hint: '节点名称'),
                Row(
                  children: [
                    Expanded(flex: 3, child: _buildTextField(controller: _addressController, label: '地址', hint: '域名或IP')),
                    const SizedBox(width: 12),
                    Expanded(flex: 1, child: _buildTextField(controller: _portController, label: '端口', hint: '443', keyboardType: TextInputType.number)),
                  ],
                ),

                if (_protocol == 'vmess' || _protocol == 'vless')
                  _buildTextField(controller: _uuidController, label: 'UUID', hint: 'User ID'),
                
                if (_protocol == 'vmess')
                  _buildDropdown<String>(
                    label: '加密方式 (Security)',
                    value: _vmessSecurity,
                    items: {'auto', 'aes-128-gcm', 'chacha20-poly1305', 'none', 'zero', _vmessSecurity}.toList(),
                ...
                if (_protocol == 'trojan' || _protocol == 'shadowsocks' || _protocol == 'socks' || _protocol == 'http')
                  _buildTextField(controller: _passwordController, label: '密码', hint: 'Password'),

                if (_protocol == 'socks' || _protocol == 'http')
                  _buildTextField(controller: _usernameController, label: '用户名', hint: 'Optional'),
                  ...
                  _buildDropdown<String>(
                    ...
                    items: [
                      ...
                      if (![
                        ...
                        '2022-blake3-aes-128-gcm', '2022-blake3-aes-256-gcm',
                        '2022-blake3-chacha20-poly1305'
                      ].contains(_ssMethod)) _ssMethod
                    ],
                    onChanged: (v) => setState(() => _ssMethod = v!),
                  ),

                if (_protocol == 'vless')
                  _buildTextField(controller: _flowController, label: '流控 (Flow)', hint: 'xtls-rprx-vision'),

                if (_protocol == 'hysteria2') ...[
                  _buildTextField(controller: _passwordController, label: '密码 (Auth)', hint: 'Auth/Password'),
                  _buildTextField(controller: _hostController, label: '伪装域名 (SNI)', hint: 'Optional'),
                  _buildDropdown<String>(
                    label: '允许不安全连接 (Insecure)',
                    value: _type == 'insecure' ? 'true' : 'false',
                    items: ['true', 'false'],
                    onChanged: (v) => setState(() => _type = v == 'true' ? 'insecure' : 'none'),
                  ),
                ],
                ...
                if (_protocol == 'tuic') ...[
                  _buildTextField(controller: _uuidController, label: 'UUID', hint: 'User ID'),
                  _buildTextField(controller: _passwordController, label: '密码', hint: 'Password'),
                  _buildTextField(controller: _sniController, label: 'SNI', hint: 'Optional'),
                  _buildDropdown<String>(
                    label: 'UDP 转发模式',
                ...
                if (_protocol == 'wireguard') ...[
                  _buildTextField(controller: _wgSecretKeyController, label: '私钥 (Secret Key)', hint: 'WireGuard Private Key'),
                  _buildTextField(controller: _wgPeerPublicKeyController, label: '对端公钥 (Peer Public Key)', hint: 'WireGuard Peer Public Key'),
                  _buildTextField(controller: _wgPreSharedKeyController, label: '预共享密钥 (Pre-shared Key)', hint: 'Optional'),
                  _buildTextField(controller: _wgLocalAddressController, label: '本地地址', hint: '10.0.0.2/32'),
                  Row(
                    children: [
                      Expanded(child: _buildTextField(controller: _wgMtuController, label: 'MTU', hint: '1420', keyboardType: TextInputType.number)),
                      const SizedBox(width: 12),
                      Expanded(child: _buildTextField(controller: _wgKeepAliveController, label: '保活间隔', hint: '20', keyboardType: TextInputType.number)),
                    ],
                  ),
                ],
                ...
                _buildDropdown<String>(
                  label: '传输方式',
                  value: _network,
                  items: {'tcp', 'ws', 'grpc', 'h2', 'kcp', 'mkcp', 'quic', 'xhttp', 'httpupgrade', 'split-http', _network}.toList(),
                ...
                if (_network == 'tcp') ...[
                  ...
                  if (_type == 'http') ...[
                    _buildTextField(controller: _hostController, label: '伪装域名 (Host)', hint: 'Optional'),
                    _buildTextField(controller: _pathController, label: '路径 (Path)', hint: '/'),
                  ],
                ],

                if (_network == 'ws' || _network == 'h2' || _network == 'httpupgrade' || _network == 'split-http' || _network == 'quic' || _network == 'xhttp') ...[
                  _buildTextField(controller: _hostController, label: '伪装域名 (Host)', hint: 'Optional'),
                  _buildTextField(controller: _pathController, label: '路径 (Path)', hint: '/'),
                ],

                if (_network == 'grpc') ...[
                  _buildTextField(controller: _serviceNameController, label: '服务名称', hint: 'grpc service name'),
                  _buildDropdown<String>(
                    label: 'gRPC 模式',
                    value: _mode,
                    items: ['gun', 'multi'],
                ...
                if (_network == 'kcp' || _network == 'mkcp') ...[
                  _buildTextField(controller: _pathController, label: '混淆种子 (Finalmask/Seed)', hint: 'Optional'),
                  Row(
                    children: [
                      Expanded(child: _buildTextField(controller: TextEditingController(text: '1350'), label: 'MTU', hint: '1350', keyboardType: TextInputType.number, readOnly: true)),
                      const SizedBox(width: 12),
                      Expanded(child: _buildTextField(controller: TextEditingController(text: '20'), label: 'TTI', hint: '20', keyboardType: TextInputType.number, readOnly: true)),
                    ],
                  ),
                  _buildDropdown<String>(
                    label: '伪装类型 (Header Type)',
                    value: _type,
                    items: {'none', 'srtp', 'utp', 'wechat-video', 'dtls', 'wireguard', 'mkcp-original', 'mkcp-aes128gcm', _type}.toList(),
                    onChanged: (v) => setState(() => _type = v!),
                  ),
                ],

                if (_protocol == 'vmess' || _protocol == 'vless' || _protocol == 'trojan' || _protocol == 'shadowsocks') ...[
                  const SizedBox(height: 16),
                  ...
                  if (_security != 'none') ...[
                    _buildTextField(controller: _sniController, label: 'SNI', hint: 'Server Name Indication'),
                    _buildDropdown<String>(
                      label: '指纹 (Fingerprint)',
                      value: _fingerprint,
                      items: {'chrome', 'firefox', 'safari', 'ios', 'android', 'edge', 'random', 'randomized', _fingerprint}.toList(),
                      onChanged: (v) => setState(() => _fingerprint = v!),
                    ),
                    _buildTextField(controller: _alpnController, label: 'ALPN (以逗号分隔)', hint: 'h2,http/1.1'),
                  ],
                  if (_security == 'reality') ...[
                    _buildTextField(controller: _publicKeyController, label: 'Reality Public Key', hint: 'Public Key'),
                    _buildTextField(controller: _shortIdController, label: 'Reality Short ID', hint: 'Optional'),
                    _buildTextField(controller: _spiderXController, label: 'SpiderX', hint: 'Optional'),
                  ],
                ],

                if (_protocol == 'vmess' || _protocol == 'vless' || _protocol == 'trojan') ...[
                  const SizedBox(height: 16),
                  _buildSectionTitle('Mux 配置'),
                  SwitchListTile(
                    title: const Text('启用 Mux', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                    value: _muxEnabled,
                    onChanged: (v) => setState(() => _muxEnabled = v),
                    contentPadding: EdgeInsets.zero,
                  ),
                  if (_muxEnabled)
                    _buildTextField(controller: _concurrencyController, label: '并发数 (Concurrency)', hint: '8', keyboardType: TextInputType.number),
                ],
        ...
        actions: [
          TextButton(
            ...
            child: Text('取消', style: TextStyle(color: Colors.grey.shade600, fontWeight: FontWeight.w700)),
          ),
          ElevatedButton(
            onPressed: _save,
  ...
  void _save() {
    ...
    final newNode = NodeModel(
      ...
      mode: _mode,
      encryption: _protocol == 'vmess' ? _vmessSecurity : (_protocol == 'vless' ? _vlessEncryption : _encryptionController.text),
      wgSecretKey: _wgSecretKeyController.text,
      wgLocalAddress: _wgLocalAddressController.text.isNotEmpty ? _wgLocalAddressController.text.split(',') : null,
      wgPeerPublicKey: _wgPeerPublicKeyController.text,
      wgPreSharedKey: _wgPreSharedKeyController.text.isNotEmpty ? _wgPreSharedKeyController.text : null,
      wgMtu: int.tryParse(_wgMtuController.text),
      wgKeepAlive: int.tryParse(_wgKeepAliveController.text),
      alpn: _alpnController.text.isNotEmpty ? _alpnController.text.split(',').map((e) => e.trim()).toList() : null,
