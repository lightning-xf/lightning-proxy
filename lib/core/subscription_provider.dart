...
class SubscriptionNotifier extends StateNotifier<List<SubscriptionModel>> {
  ...
  void updateSubscription(String id) async {
    ...
    try {
      ...
      final bool wasRunning = vpnState.isRunning;
      final NodeModel? currentNode = _ref.read(selectedNodeProvider);

      if (wasRunning) {
        await _ref.read(vpnProvider.notifier).stopProxy();
        // 等待进程真正退出释放文件锁（Windows 平台）
        final vpnManager = _ref.read(vpnManagerProvider);
        if (Platform.isWindows && vpnManager is dynamic) {
          try {
            await (vpnManager as dynamic).whenExited;
          } catch (e) {
            // 回退到硬编码延迟
      ...
      if (wasRunning && currentNode != null) {
        final updatedNodes = _ref.read(nodeProvider);
        final newNode = updatedNodes.firstWhere(
        ...
        _ref.read(vpnProvider.notifier).toggleVpn(newNode);
      }
