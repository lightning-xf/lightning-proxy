...
class S {
  ...
  static final Map<String, Map<String, String>> _localizedValues = {
    'en': {
      ...
      'mux_advanced': 'Mux Advanced',
      'mux_concurrency': 'Concurrency',
      'protocol': 'Protocol',
      ...
      'use_proxy_group': 'Proxy Group Mode',
      'use_proxy_group_desc': 'Use advanced proxy groups instead of a single node',
      'proxy_ip_hint': 'Set Proxy IP to: {ip}',
      
      // Feature Details (for Info Icon)
      'use_proxy_group_help': 'Connect using proxy groups (Select, URL Test, Load Balance, etc.). Off means connecting to a single node directly.',
      'mux_enabled_help': 'Multiplexing allows multiple streams over a single connection. It reduces handshake latency but might be unstable on some networks.',
      'fake_dns_help': 'Uses fake IP addresses for DNS resolution. It significantly speeds up connection and prevents DNS pollution.',
      'bypass_local_help': 'Excludes local network traffic from the proxy. Keep this ON to access local devices like printers.',
      'tcp_congestion_help': 'Algorithm to control data flow. BBR is recommended for most users as it usually provides better speeds.',
      'allow_lan_help': 'Allows other devices on your local network to use this phone as a proxy gateway.',
      'ipv6_help': 'Enable or disable IPv6 traffic support.',
      'proxy_group_help': 'Manage your proxy groups and strategies.',
      'rule_set_help': 'Manage remote rule sets for automatic updates.',
      'routing_rules_help': 'Manage custom routing rules for domains and IPs.',
      'app_splitting_help': 'Control which apps use the proxy and which bypass it.',
      'mux_advanced_help': 'Configure advanced Mux parameters like concurrency and protocol.',
      'inbound_ports_help': 'Set the local ports for Socks and HTTP proxy services.',
      'dns_settings_help': 'Configure DNS servers and Fake DNS behavior.',
      'routing_strategy_help': 'Global: All traffic through proxy. Rule: Intelligent routing based on rules. Direct: No proxy.',
      'backup_restore_help': 'Export your current settings to clipboard or import from one.',
      'about_help': 'Learn more about Lightning and its development team.',
      ...
      'keep_alive_guide_content': 'To ensure proxy service runs stably in the background, please complete the following settings:',
      'keep_alive_step1': 'Allow auto-start',
      'keep_alive_step2': 'Disable battery optimization (Set to Unrestricted)',
      'keep_alive_step3': 'Lock Lightning in recent apps screen',
      'keep_alive_step4': 'Allow background activity',
      'auto_start_help':
      ...
      'proxy_group_desc': 'Manage your proxy groups and strategies',
      'no_groups': 'No Proxy Groups',
      'create_group_hint': 'Create your first proxy group',
      'nodes': 'nodes',
      'select': 'Select',
      'url_test': 'URL Test',
      'fallback': 'Fallback',
      ...
      'rule_sets': 'Rule Sets',
      'no_rule_sets': 'No Rule Sets',
      'create_rule_set_hint': 'Add your first rule set subscription',
      'domain': 'Domain',
      'ip_cidr': 'IP CIDR',
      'classical': 'Classical',
      'format': 'Format',
      ...
      'rule_set_url': 'Rule Set URL',
      'create_rule_set': 'Create Rule Set',
      'edit_rule_set': 'Edit Rule Set',
      'delete_rule_set': 'Delete Rule Set',
      'delete_rule_set_confirm': 'Are you sure you want to delete',
      'last_updated': 'Last Updated',
      'update': 'Update',
      'updating': 'Updating',
      'updated': 'updated successfully',
      'preset_rule_sets': 'Preset Rule Sets',
      'add_preset_rule_set': 'Add Preset Rule Set',
      'backup_export': 'Backup Config (Export)',
      'backup_import': 'Restore Config (Import)',
      'backup_success': 'Configuration backed up to clipboard',
      'backup_fail': 'Backup failed: {error}',
      'restore_success': 'Configuration restored successfully, please restart the app',
      'restore_fail': 'Import failed: Invalid configuration format',
      'mux_concurrency_label': 'Mux Concurrency',
      'mux_protocol_label': 'Mux Protocol',
      'mux_padding_label': 'Enable Traffic Padding',
      'default_hint': 'Default {value}',
      'inbound_settings': 'Inbound Port Settings',
      'socks_port_label': 'Socks Port',
      'http_port_label': 'HTTP Port',
      'about_lightning': 'About Lightning',
      'core_technology': 'Core Technology',
      'based_on_xray': 'Built on Xray-core',
      ...
      'select_group_first': 'Please select a proxy group first',
    ...
    'zh': {
      ...
      'use_proxy_group': '代理组模式',
      'use_proxy_group_desc': '使用高级代理组（负载均衡、自动切换等）',
      'proxy_ip_hint': '请在其他设备上设置代理 IP 为：{ip}',
      
      'use_proxy_group_help': '功能：在“代理组模式”与“单节点模式”之间切换。\n\n开启（代理组）：你可以使用包含多个节点的策略组（如手动选择、负载均衡、自动延迟测试切换等），适合对网络灵活性要求高的场景。\n\n关闭（单节点）：直接连接你所选中的单个服务器节点，逻辑更简单，性能损耗更低，适合节点稳定的场景。',
      
      'proxy_group_help': '功能：管理你的代理组。你可以创建不同策略的代理组，例如自动选择最快节点、负载均衡或手动切换。',
      'rule_set_help': '功能：管理远程规则集订阅。规则集可以自动更新，让你始终拥有最新的分流策略（如绕过中国网站、拦截广告等）。',
      'routing_rules_help': '功能：管理自定义路由规则。你可以手动添加特定域名或 IP 的直连/代理规则。',
      'app_splitting_help': '功能：控制哪些 App 走代理，哪些 App 直连。解决某些银行或本地 App 无法在代理下正常运行的问题。',
      'mux_advanced_help': '功能：设置 Mux 多路复用的并发数和协议。通常默认设置即可，在弱网环境下调低并发可能更稳定。',
      'inbound_ports_help': '功能：设置本地 Socks 和 HTTP 服务的监听端口。修改端口后，请确保其他手动设置了代理的应用也同步更新。',
      'dns_settings_help': '功能：配置 DNS 解析。你可以设置远程解析服务器和国内解析服务器，并开启 Fake DNS 以获得极速体验。',
      'backup_restore_help': '功能：导出所有设置（包括节点和规则）到剪贴板，或从剪贴板恢复。方便在不同设备间同步配置。',
      'about_help': '关于 Lightning：了解我们的愿景、团队以及如何参与贡献。',
      ...
      'keep_alive_guide_content': '为了确保代理服务在后台稳定运行，请确保完成以下设置：',
      'keep_alive_step1': '允许应用自启动',
      'keep_alive_step2': '关闭电池优化（设为不限制）',
      'keep_alive_step3': '在多任务界面锁定 Lightning',
      'keep_alive_step4': '允许后台常驻运行',
      'auto_start_help':
      ...
      'proxy_groups': '代理组',
      'proxy_group': '代理组',
      'proxy_group_desc': '管理您的代理组和策略',
      'no_groups': '暂无代理组',
      'create_group_hint': '创建您的第一个代理组',
      'nodes': '个节点',
      'select': '手动选择',
      'url_test': 'URL 测速',
      'fallback': '故障转移',
      ...
      'enabled': '启用',
      'select_nodes': '选择节点',
      'no_nodes_available': '没有可用节点',
      'create_group': '创建组',
      'edit_group': '编辑组',
      'delete_group': '删除组',
      'delete_group_confirm': '确定要删除',
      'rule_sets': '规则集',
      'no_rule_sets': '暂无规则集',
      'create_rule_set_hint': '添加您的第一个规则集订阅',
      'domain': '域名',
      'ip_cidr': 'IP 段',
      'classical': '经典',
      'format': '格式',
      'behavior': '行为',
      'update_rule_set': '更新规则集',
      'update_interval_hours': '更新间隔（小时）',
      'optional': '可选',
      'rule_set_name': '规则集名称',
      'rule_set_url': '规则集 URL',
      'create_rule_set': '创建规则集',
      'edit_rule_set': '编辑规则集',
      'delete_rule_set': '删除规则集',
      'delete_rule_set_confirm': '确定要删除',
      'last_updated': '最后更新',
      'update': '更新',
      'updating': '正在更新',
      'updated': '更新成功',
      'preset_rule_sets': '预设规则集',
      'add_preset_rule_set': '添加预设规则集',
      'backup_export': '备份配置 (导出)',
      'backup_import': '恢复配置 (导入)',
      'backup_success': '配置已备份至剪贴板',
      'backup_fail': '备份失败: {error}',
      'restore_success': '配置恢复成功，请重启应用生效',
      'restore_fail': '导入失败: 无效的配置格式',
      'mux_concurrency_label': '并发连接数',
      'mux_protocol_label': '协议类型',
      'mux_padding_label': '启用流量填充 (Padding)',
      'default_hint': '默认 {value}',
      'inbound_settings': '入站端口设置',
      'socks_port_label': 'Socks 端口',
      'http_port_label': 'HTTP 端口',
      'about_lightning': '关于 Lightning',
      'core_technology': '核心技术',
      'based_on_xray': '基于 Xray-core 构建',
