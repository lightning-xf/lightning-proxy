...
class LinkParser {
        ...
        if (decoded.trim().isNotEmpty) {
          ...
          for (var line in decoded.split(RegExp(r'[\n\r]+'))) {
            ...
            if (l.startsWith('STATUS=') ||
                ...
                l.startsWith('INTERVAL=')) continue;
            filteredLines.add(l);
          }
  ...
  static NodeModel _parseSocks(String link) {
    ...
    return NodeModel(
      ...
      rawData: query,
      security:
          query['tls'] == '1' || query['security'] == 'tls' ? 'tls' : 'none',
    );
  }
  ...
  static NodeModel _parseTuic(String link) {
    ...
    return NodeModel(
      ...
      sni: query['sni'],
      network: query['alpn'],
      type: (query['insecure'] == '1' || query['allowInsecure'] == '1')
          ? 'insecure'
          : null,
      rawData: query,
    );
    ...
    if (type == 'vmess') {
      ...
      nodes.add(
        NodeModel(
          ...
          wgLocalAddress:
              data['ip']?.toString() != null ? [data['ip']!.toString()] : null,
  ...
  static List<NodeModel> _parseXrayJson(Map<String, dynamic> data) {
    ...
    try {
      final List<dynamic> outbounds =
          data.containsKey('outbounds') ? data['outbounds'] : [data];
        ...
        if (protocol == 'vmess' || protocol == 'vless') {
          ...
          if (vnext != null && vnext.isNotEmpty) {
            ...
            if (users != null && users.isNotEmpty) {
              ...
              encryption = user['security']?.toString() ??
